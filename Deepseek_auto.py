...
# 2 : 超时（120 秒内没找到复制按钮）
# 3 : 模板图片缺失
# 5 : 【新增】检测到deepseek拒答 -> AppleScript 收到后终止deepseek流程，改用 Doubao_News.scpt 继续
EXIT_OK = 0
EXIT_UNQUALIFIED = 1
EXIT_TIMEOUT = 2
EXIT_TEMPLATE_MISSING = 3
EXIT_SWITCH_TO_DOUBAO = 5
...
def find_image_on_screen(template, screen=None, threshold=0.9):
    """
    在屏幕上查找图片
    screen 参数：如果传入了截屏数据，则直接使用，避免重复截屏提高效率
...
def is_refusal_response(text: str) -> bool:
    """
    判断 deepseek 是否返回了拒答/无法处理内容。
    命中后不再重试、也不再跳过文章，而是通知 AppleScript 切换到豆包。
    """
    if not text:
        return False
...
def main():
    ...
    # ==== 1. 加载所有模板 ====
    template_copy_path = os.path.join(BASE_RESOURCE_DIR, "deepseek_copy.png")
    template_forbidden_path = os.path.join(BASE_RESOURCE_DIR, "deepseek_forbidden.png")
    template_forbidden2_path = os.path.join(BASE_RESOURCE_DIR, "deepseek_forbidden2.png")
    template_retry_path = os.path.join(BASE_RESOURCE_DIR, "deepseek_retry.png")
    template_timeout_path = os.path.join(BASE_RESOURCE_DIR, "deepseek_timeout.png")
    ...
    for name, path in required_templates:
        if not os.path.exists(path):
            ...
            sys.exit(EXIT_TEMPLATE_MISSING)  # 模板缺失错误
    ...
    while current_attempt <= max_attempts:
        ...
        if time.time() - start_time > timeout_duration:
            ...
            sys.exit(EXIT_TIMEOUT)  # 状态码 2：超时退出

        # 截取当前屏幕，供后续所有模板共用，提高匹配效率
        current_screen = capture_screen()

        # 1. 优先寻找 Forbidden / Forbidden2 按钮（任意一个匹配都视为拒答）
        ...
        if forbidden_loc or forbidden2_loc:
            found_name = "deepseek_forbidden" if forbidden_loc else "deepseek_forbidden2"
            print(f"检测到 {found_name} 图片，判定为deepseek拒答 -> 请求切换到豆包。")
            sys.exit(EXIT_SWITCH_TO_DOUBAO)
        ...
        if location:
            ...
            # ============================================================
            # 【核心修改】剪贴板命中拒答话术 -> 退出码 5
            # AppleScript 会据此终止deepseek流程，改用 Doubao_News.scpt 继续
            # （既不写 copy_failure.html，也不跳过这篇新闻）
            # ============================================================
            if is_refusal_response(content):
                print("检测到deepseek拒答内容 -> 请求切换到豆包继续处理本篇文章。")
                sys.exit(EXIT_SWITCH_TO_DOUBAO)
            ...
            # 没找到 Copy / Retry / Timeout / Forbidden，滚动屏幕继续寻找
            pyautogui.scroll(SCROLL_AMOUNT)
            sleep(1)
