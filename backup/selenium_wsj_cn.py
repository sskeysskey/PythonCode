...
# 3. 浏览器与驱动路径 (跨平台适配)
# 注意：你需要确保目标电脑上安装了对应版本的 Chrome 和 ChromeDriver
if platform.system() == 'Darwin':
    # macOS 配置 (保持你原有的 Beta 版设置)
    CHROME_BINARY_PATH = "/Applications/Google Chrome Beta.app/Contents/MacOS/Google Chrome Beta"
    # 假设 ChromeDriver 放在 Downloads/backup 下
    CHROME_DRIVER_PATH = os.path.join(DOWNLOADS_DIR, "backup", "chromedriver_beta")
elif platform.system() == 'Windows':
    # Windows 配置 (默认使用标准版 Chrome，通常会自动找到，也可以指定路径)
    ...
    # Windows 下驱动通常叫 chromedriver.exe
...
# 4. 业务文件路径
OLD_FILE_PATTERN = os.path.join(BASE_CODING_DIR, "News", "backup", "site", "wsj_cn.html")
NEW_HTML_PATH = os.path.join(BASE_CODING_DIR, "News", "backup", "site", "wsj_cn.html")
TODAY_HTML_PATH = os.path.join(BASE_CODING_DIR, "News", "today_wsjcn.html")
...
def main():
    ...
    if os.path.exists(CHROME_BINARY_PATH):
        ...
        # 如果不设置 binary_location，Selenium 会尝试自动查找
    ...
    try:
        ...
        for href, title_text in raw_data_list:
            if 'cn.wsj.com' not in href: continue
            if any(x in href for x in ['podcasts', 'sports', 'buyside']): continue
