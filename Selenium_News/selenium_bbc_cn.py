...
# 4. 业务文件路径 (修改为 BBC)
OLD_FILE_PATTERN = os.path.join(BASE_CODING_DIR, "News", "backup", "site", "bbc_cn.html")
NEW_HTML_PATH = os.path.join(BASE_CODING_DIR, "News", "backup", "site", "bbc_cn.html")
TODAY_HTML_PATH = os.path.join(BASE_CODING_DIR, "News", "today_bbccn.html")

# ========================================================

def is_similar(url1, url2):
    """
    比较两个 URL 的相似度 (针对 BBC 优化：提取 /articles/ 后面的唯一 ID)
    """
    ...
    def get_core_id(url):
        ...
        # 尝试提取 BBC 文章 ID，例如 /articles/cp81reg5pepo
        match = re.search(r'/articles/([a-zA-Z0-9]+)', path)
...
def main():
    ...
    try:
        ...
        selectors = [
            "//div[contains(@class, 'promo-text')]//h3/a",
            "//h3/a"
        ]
            ...
            try:
                ...
                for element in elements:
                    try:
                        ...
                        # 提取文本，去掉可能存在的换行和多余空格
                        title_text = element.text.strip().replace('\n', ' ')
        ...
        print("-" * 40)
        if new_rows:
            print(f"✅ 统计报告: 本次共抓取到 {len(new_rows)} 条 BBC 新闻！")
        else:
