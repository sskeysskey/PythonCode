...
# 2 : 超时（120 秒内没找到复制按钮）
# 3 : 模板图片缺失
# 5 : 【新增】检测到千问拒答 -> AppleScript 收到后终止千问流程，改用 Doubao_News.scpt 继续
EXIT_OK = 0
EXIT_UNQUALIFIED = 1
EXIT_TIMEOUT = 2
...
def main():
    ...
    while current_attempt <= max_attempts:
        ...
        if forbidden_loc or forbidden2_loc:
            ...
            print(f"检测到 {found_name} 图片，判定为千问拒答 -> 请求切换到豆包。")
        ...
        if location:
            ...
            # AppleScript 会据此终止千问流程，改用 Doubao_News.scpt 继续
            # （既不写 copy_failure.html，也不跳过这篇新闻）
            # ============================================================
            if is_refusal_response(content):
                print("检测到千问拒答内容 -> 请求切换到豆包继续处理本篇文章。")
                sys.exit(EXIT_SWITCH_TO_DOUBAO)
