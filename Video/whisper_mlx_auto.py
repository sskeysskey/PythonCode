...
if __name__ == "__main__":
    ...
    media_files = sorted(
        p for p in video_source_dir.iterdir()
        if p.is_file() and p.suffix.lower() in MEDIA_EXTS
    )

    if not media_files:
        logging.warning("在选定目录中没有找到任何受支持的媒体文件。")
    else:
        logging.info(f"发现 {len(media_files)} 个媒体文件，开始处理...")
        done, skipped, failed = 0, 0, 0

        for i, media_path in enumerate(media_files, start=1):
            srt_path = media_path.with_suffix('.srt')
            if srt_path.exists():
                logging.info(f"[{i}/{len(media_files)}] 已存在字幕，跳过: {media_path.name}")
                skipped += 1
                continue

            logging.info(f"[{i}/{len(media_files)}] 处理: {media_path.name}")
            if run_pipeline(str(media_path), "large-v3", None):
                done += 1
            else:
                failed += 1

        logging.info(f"统计：成功 {done}，跳过 {skipped}，失败 {failed}。")
